...
def getPeopleCount(actions):
    ...
    # Here, I can either track prevTimestamp, or a running count
    # Using prevTimestamp, I can access the most recent count from the countsAtTime
    # dict, or I can track the running count and just write that to the dict
    # using the current timestamp, and not worry about accessing the dict a bunch.
    # There's not a huge difference, but the running count is just a bit simpler.
    # It removes the necessity for handling the initial None case of using timestamps

    # I'm not going to bother with edge cases, like where more people exit than
    # have entered.  That's for a different system to deal with.
    # Realizing I can make the logic a bit simpler using an actionHandler dict.
    # It's overkill, but fun.
    runningCount = 0
    for actionDict in actions:
        ...
        countsAtTime[timestamp] = runningCount

    maxPeople = [0, float("-inf")]

    for entry in countsAtTime:
        count = countsAtTime[entry]
        if count > maxPeople[1]:
            maxPeople = [entry, count]
